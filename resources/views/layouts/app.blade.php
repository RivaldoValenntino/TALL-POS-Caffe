...
<body x-data="{ loading: true }" x-init="setTimeout(() => loading = false, 2000)">
    <div x-show="loading" class="loader z-[999]" x-transition:enter="transition-opacity ease-out duration-500"
        ...
        <div class="bg-black bg-opacity-70 fixed inset-0 flex items-center justify-center h-screen z-[999] w-full">
    ...
    <div>
        <livewire:header />
        <div class="w-full py-12 dark:bg-neutral-800 bg-slate-100 h-svh">
            <div class="pt-8 space-y-4 sm:space-y-6">
                {{ $slot }}
            </div>
        </div>
    </div>
